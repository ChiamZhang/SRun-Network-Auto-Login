...
echo "AC_ID: $AC_ID (来自 config 的 ACID)"

REF_PORTAL_PC="https://gw.buaa.edu.cn/srun_portal_pc?ac_id=${AC_ID}&theme=${BUAA_THEME}&url=www.buaa.edu.cn&srun_domain="

# get_challenge 的 ip：与 zzdyyy/buaa_gateway_login 一致用 0.0.0.0，由网关返回真实 client_ip；也可在 config 里设 IPADDR 覆盖
CHALLENGE_IP="${IPADDR:-0.0.0.0}"
...
-H "Accept-Language: en-US,en;q=0.9,zh-CN;q=0.8,zh;q=0.7,zh-TW;q=0.6" \
"https://gw.buaa.edu.cn/cgi-bin/get_challenge?callback=jQuery112407419864172676014_1566720734115&username="$USERNAME"&ip="$CHALLENGE_IP"&_="$TIMESTAMP`
...
if [[ "$option" == "login" ]]; then
	...
	-H "Referer: $REF_PORTAL_PC" \
	-H "Accept-Language: en-US,en;q=0.9,zh-CN;q=0.8,zh;q=0.7,zh-TW;q=0.6" \
	"https://gw.buaa.edu.cn/cgi-bin/srun_portal?callback=jQuery112407419864172676014_1566720734115&action=login&username="$USERNAME"&password=%7BMD5%7D"$PWD"&ac_id=$AC_ID&ip="$CLIENTIP"&chksum="$CHKSUM"&info=%7BSRBX1%7D"$URL_INFO"&n=200&type=1&os="$SYSNAME"&name=Macintosh&double_stack="$DS_STACK"&_="$TIMESTAMP)
