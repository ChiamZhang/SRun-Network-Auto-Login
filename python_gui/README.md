...
运行仓库根目录的 `srun_gui.py`：

```shell
python3 srun_gui.py
```

Windows 可双击 `srun_gui.py`，或在 PowerShell 中运行 `py srun_gui.py`。Linux、macOS、Windows 均只需要 Python 3（自带 `tkinter`）；不需要 Bash、curl 或第三方 Python 包。

首次运行没有 `config` 文件时，在窗口中粘贴浏览器里的登录成功页 URL（例如 `https://***.****.edu.cn/srun_portal_success?ac_id=1`），填写学号和密码并点击“保存配置”。程序会自动拆分协议、网关主机和 `ac_id`，生成与命令行脚本兼容的 `config`。之后直接点击“登录”即可完成认证；“注销”发送下线请求。密码只保存在本机的 `config` 文件中，请勿提交到公开仓库。

窗口中的“指定网卡”可选择当前系统的网络接口；Linux/macOS 会尝试读取该接口的 IPv4 地址，Windows 使用系统路由。点击“后台保活”后程序会周期检测认证状态，掉线或接入网络后自动登录，再次点击即可停止。
...
```shell
python3 srun_gui.py login                 # 登录
python3 srun_gui.py logout                # 注销
python3 srun_gui.py try                   # 未在线时登录
python3 srun_gui.py protect               # 后台循环保活（Ctrl+C 停止）
python3 srun_gui.py login --interface en0 # 临时指定网卡
python3 srun_gui.py login --acid 67       # 临时指定 ACID
```

### 打包为可点击软件

在目标系统上安装一次 PyInstaller，然后运行 `python build.py`：
...
生成物位于 `dist/SRunLogin`（Linux/macOS）或 `dist/SRunLogin.exe`（Windows）。需要同时支持三种系统时，分别在对应系统上构建；PyInstaller 会把 Python 解释器和依赖一起打包。`config` 放在可执行文件同目录，首次启动仍会弹出配置界面。
